...
Write-Host "`nToolDock Release Script" -ForegroundColor Cyan
...
Write-Host "Updating tauri.conf.json..." -ForegroundColor Green
...
Write-Host "`nVersion updated: $oldVersion -> $Version" -ForegroundColor Green
...
$tagName = "v$Version"
Write-Host "`nCreating tag $tagName..." -ForegroundColor Green

# Check if tag already exists
$existingTag = git tag -l $tagName
if ($existingTag) {
    Write-Host "Tag $tagName already exists, replacing..." -ForegroundColor Yellow
    git tag -d $tagName
...
Write-Host "`nRelease v$Version created successfully!" -ForegroundColor Cyan
