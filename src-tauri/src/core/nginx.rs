...
pub fn create_backup(path: PathBuf) -> AppResult<String> {
    ...
    let filename = path.file_name().ok_or_else(|| crate::errors::AppError::Internal("Invalid filename".into()))?.to_string_lossy();
...
pub fn list_backups(path: PathBuf) -> AppResult<Vec<BackupInfo>> {
    let parent = path.parent().ok_or_else(|| crate::errors::AppError::Internal("Invalid path".into()))?;
    let file_name = path.file_name().ok_or_else(|| crate::errors::AppError::Internal("Invalid filename".into()))?.to_string_lossy();
    let file_stem = path.file_stem().ok_or_else(|| crate::errors::AppError::Internal("Invalid filename".into()))?.to_string_lossy();
    ...
    if let Ok(entries) = fs::read_dir(parent) {
        for entry in entries.flatten() {
            ...
            if path.is_file() {
                ...
                if name.starts_with(&format!("{}.bak.", file_name)) || name.starts_with(&format!("{}.bak.", file_stem)) {
                    if let Ok(metadata) = entry.metadata() {
                        let created_at = metadata.modified().unwrap_or_else(|_| metadata.created().unwrap());
...
pub fn start_nginx(path: PathBuf, nginx_path: Option<String>) -> AppResult<()> {
    ...
    command.spawn().map_err(|e| crate::errors::AppError::Internal(format!("Failed to start Nginx: {}", e)))?;
    Ok(())
}
...
pub fn stop_nginx(nginx_path: Option<String>) -> AppResult<()> {
    ...
    match output {
        ...
        _ => {
            // 如果 nginx -s stop 失败，尝试使用 taskkill
            Command::new("taskkill")
                .arg("/F")
                .arg("/IM")
                .arg("nginx.exe")
                .output()
                .map_err(|e| crate::errors::AppError::Internal(format!("Failed to stop Nginx: {}", e)))?;
            Ok(())
        }
    }
}
...
pub fn scan_configs() -> Vec<String> {
    ...
    // 也可以尝试从环境变量中查找 nginx.exe，然后推断配置路径
    if let Ok(path_env) = std::env::var("PATH") {
        for p in std::env::split_paths(&path_env) {
            let nginx_exe = p.join("nginx.exe");
            if nginx_exe.exists() {
