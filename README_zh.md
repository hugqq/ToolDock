...
- 29 个实用工具，加上一个设置中心，按文件、系统、网络、开发、娱乐分类管理。
- 全局快捷键直接打开命令面板，可同时搜索 ToolDock 工具和本地文件。
...
| 本地文件搜索 | Windows 需要安装并运行 Everything 1.4+（启用 IPC）；macOS 使用系统 Spotlight |

Everything 仅用于 Windows 文件搜索，ToolDock 不会自动下载或安装它；未安装时，命令面板中的工具搜索仍可正常使用。
...
- HTTP/API 调试器：发送 GET/POST 等请求，编辑请求头、查询参数和 JSON/表单/文本请求体，并查看响应、耗时、历史及 cURL。
